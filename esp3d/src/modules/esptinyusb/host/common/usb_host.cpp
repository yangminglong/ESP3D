...
void _client_event_callback(const usb_host_client_event_msg_t *event_msg, void *arg)
{
    ...
    {
        ESP_LOGI("USBhost", "client event: %d, address: %d", event_msg->event, event_msg->new_dev.address);
...
bool USBhost::init(bool create_tasks)
{
    ...
    ESP_LOGI("USBhost", "install status: %d", err);
    ...
    ESP_LOGI("USBhost", "client register status: %d", err);
...
const usb_device_desc_t* USBhost::getDeviceDescriptor()
{
    ...
    return usb_host_get_device_descriptor(dev_hdl, &device_desc) == ESP_OK ? device_desc : nullptr;
}
