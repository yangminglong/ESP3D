...
#if defined(WEBDAV_FEATURE) && WEBDAV_ASYNC==1
...
bool AsyncWebdav::canHandle(AsyncWebServerRequest *request)
{
    ...
    Serial.print("canHandle: ");
    Serial.print(request->url());
    Serial.print(" -> ");
    Serial.print(this->_url);
    Serial.println();
    ...
    if (request->url().startsWith(this->_url))
    {
        ...
        if (request->method() == HTTP_GET || request->method() == HTTP_HEAD || request->method() == HTTP_OPTIONS || request->method() == HTTP_PUT || request->method() == HTTP_LOCK || request->method() == HTTP_UNLOCK || request->method() == HTTP_MKCOL || request->method() == HTTP_DELETE || request->method() == HTTP_COPY)
        {
            return true;
        }
    }
...
void AsyncWebdav::sendPropResponse(AsyncResponseStream *response, boolean recursing, File *curFile)
{
    ...
    response->printf("<d:quota-used-bytes>%d</d:quota-used-bytes>", 100);
    response->printf("<d:quota-available-bytes>%d</d:quota-available-bytes>", 100);
...
#endif // #if defined (WEBDAV_FEATURE)
